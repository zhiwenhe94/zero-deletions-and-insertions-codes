...
## Algorithm "Verification Construction III.1 for $A_t^3$"
### function $CsA3(t)$

Functionality: Used to obtain a constant weight code $A_t^{w+1}=A_t^3$ in Construction III.1(1).
...
Output: $2t$, if $A$ has constant weight $2t$; $-1$, otherwise.


### function $SizCkb(t,At3)$

Functionality: Used for checking the size of $A_t^3$.
...
## Algorithm "Verification Construction III.1 for $B_t^3$"
### function $CsB3(t)$

Functionality: Used to obtain a constant weight code $B_t^{w+1}=B_t^3$ in Construction III.1(2).
...
Output: $2t+1$, if $B$ has constant weight $2t+1$; $-1$, otherwise.
...
## Comparisons between codes $\mathcal{C}_t^{w+1}$, $A_t^{w+1}$, and $A_t^{w+1}(\mathcal{Y})$ for $w=5$
